...
class YAMLAttributeContainerDefinitionsFileTest(shared_test_lib.BaseTestCase):
    ...
    def testReadDefinition(self):
        ...
        container_class = test_definitions_file._ReadDefinition(self._FORMATTERS_YAML)

        self.assertIsNotNone(container_class)
        self.assertEqual(
            container_class.CONTAINER_TYPE, "windows_eventlog_message_file"
        )
        self.assertEqual(container_class.SCHEMA, {"path": "str", "windows_path": "str"})
        ...
        with self.assertRaises(errors.ParseError):
            test_definitions_file._ReadDefinition(
                {"name": "windows_eventlog_message_file", "attributes": []}
            )
    ...
    def testReadFromFile(self):
        ...
        self.assertEqual(definitions[0].CONTAINER_TYPE, "windows_eventlog_message_file")
